...
class TestComputeFileHash:
    ...
    def test_compute_file_hash_modified_content(self, temp_project_dir):
        ...
        ), "Computed hash should differ from stored hash"
...
class TestCopyResource:
    ...
    def test_copy_resource_basic(self, temp_project_dir):
        ...
        assert (
            "from invoker import InvokerScript" in content
    ...
    def test_copy_resource_with_signing(self, temp_project_dir):
        ...
        assert (
            "# DO NOT MANUALLY EDIT THIS FILE." in lines[1]
        ), "Should have DO NOT EDIT warning"
        # Resource path and date lines
        assert any(
            line.startswith("# Resource name: invoker.py") for line in lines[:10]
        ), "Header should contain invoker resource path"
        assert any(
            line.startswith("# Date: ") for line in lines[:10]
    ...
    def test_copy_resource_with_preprocessing(self, temp_project_dir):
        ...
        ResourceManager.import_resource(
            "module_base.py", dest_file, sign=False, preprocess_fn=replace_placeholder
        )
        ...
        assert "BaseTestModule" in content, "Should contain preprocessed class name"
...
class TestInvokerHeaderOps:
    ...
    def test_build_invoker_header_contains_core_fields(self, tmp_path):
        ...
        assert (
            "# DO NOT MANUALLY EDIT THIS FILE." in header_text
        ), "Header should contain warning"
        assert (
            "# Resource name: script.py" in header_text
        ), "Header should contain resource path"
    ...
    def test_parse_invoker_header_extracts_fields(self, tmp_path):
        ...
        assert fields.get("version") == metadata.version(
            "invoker"
        ), "Version should match package version"
        assert (
            fields.get("resource") == "script.py"
        ), "Resource should be extracted from header"
        assert fields.get("date"), "Date should be present"
        assert (
            fields.get("hash") and len(fields["hash"]) == 32
        ), "Hash should be present and 32 hex chars"
    ...
    def test_strip_invoker_header_removes_header(self, tmp_path):
        ...
        assert (
            stripped == body_lines
        ), "strip_invoker_header should return only body lines"
    ...
    def test_strip_invoker_header_no_header_returns_same(self, tmp_path):
        ...
        assert (
            stripped == body_lines
        ), "Files without header should be returned unchanged"
    ...
    def test_strip_invoker_header_skips_trailing_blank_lines(self, tmp_path):
        ...
        temp_file.write_text(
            header_text + "".join(blanks) + "".join(body_lines), encoding="utf-8"
        )

        stripped = ResourceManager.strip_invoker_header(temp_file)
        assert (
            stripped == body_lines
        ), "Trailing blank lines after header should be skipped"
