...
class TestImportResources:
    ...
    def test_cli_import_with_dest(self, temp_project_dir, monkeypatch):
        ...
        result = runner.invoke(
            cli, ["import", "util/image.py", "--dest", "assets/img.py"]
        )
    ...
    def test_cli_import_outside_resources_forbidden(
        ...
        assert (
            result.exit_code != 0
        ), "CLI should fail for resource path outside resources"
    ...
    def test_cli_import_disallows_resources_tests(self, temp_project_dir, monkeypatch):
        ...
        assert (
            result.exit_code != 0
        ), "CLI should fail for resource path under resources/tests"

    def test_import_project_overwrite_backs_up_existing_file(
        self, temp_project_dir, capsys
    ):
        project = Project(temp_project_dir)
        project.initialize()
