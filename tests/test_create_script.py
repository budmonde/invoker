...
class TestCreateScript:
    ...
    def test_create_script_content(self, temp_project_dir):
        ...
        assert (
            f"class {expected_class_name}(InvokerScript):" in content
        ), f"Should contain class definition with name {expected_class_name}"

        # Check that all occurrences of __SCRIPT__ were replaced
        assert (
            "__SCRIPT__" not in content
        ), "Template placeholder __SCRIPT__ should be replaced"

        # Check for key script components
        assert "#!/usr/bin/env python" in content, "Script should have shebang line"
        assert (
            "from invoker import InvokerScript" in content
        ), "Script should import InvokerScript class"
        assert "def args(cls):" in content, "Script should have args method"
        assert "def modules(cls):" in content, "Script should have modules method"
        assert (
            "def build_config(cls, args):" in content
        ), "Script should have build_config method"
        assert "def run(self):" in content, "Script should have run method"
        assert (
            'if __name__ == "__main__":' in content
        ), "Script should have main entry point"
        assert (
            f"{expected_class_name}(run_as_root_script=True).run()" in content
    ...
    def test_create_script_permissions(self, temp_project_dir):
        ...
        assert file_stat.st_mode & stat.S_IXUSR, "Script should be executable by owner"
        assert file_stat.st_mode & stat.S_IRUSR, "Script should be readable by owner"
        assert file_stat.st_mode & stat.S_IWUSR, "Script should be writable by owner"

        # Verify the exact mode is 0o744 (rwxr--r--)
        # Get just the permission bits
        permission_bits = stat.S_IMODE(file_stat.st_mode)
        assert (
            permission_bits == 0o744
