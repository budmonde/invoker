...
def make_array_from_shape(lib, shape, dtype=np.float32, device='cpu'):
	...
	if lib == 'numpy':
		...
		if isinstance(dtype, torch.dtype) or dtype == torch.FloatTensor:
			raise ValueError("Mismatched library usage in make_array_from_shape: numpy lib with torch dtype is unsupported")
	...
	if lib == 'torch':
		...
		if not (isinstance(dtype, torch.dtype) or dtype == torch.FloatTensor):
			raise ValueError("Mismatched library usage in make_array_from_shape: torch lib with numpy dtype is unsupported")
