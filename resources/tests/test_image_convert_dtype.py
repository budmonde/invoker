...
from .helpers import make_array_from_shape, assert_type_and_dtype
...
def test_numpy_to_torch_cpu(target_dtype, shape=(2, 3)):
	...
	assert y.dtype == (torch.float32 if target_dtype == torch.FloatTensor else target_dtype)
...
def test_torch_to_numpy(target_dtype, shape=(2, 3)):
	x = make_array_from_shape('torch', shape, dtype=(torch.float32 if target_dtype == np.float32 else torch.uint8), device='cpu')
	y = convert_dtype(x, dtype=target_dtype)
...
def test_invalid_dtype_raises(lib):
	x = make_array_from_shape(lib, (2, 3), dtype=(np.float32 if lib == 'numpy' else torch.float32))
